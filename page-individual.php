<?php include 'inc/header.php'; ?>

<?php include 'inc/mv-temp.php'; ?>

<!-- 個人様向けプランの紹介 -->
<div class="page-individual wrapper_sm">
    <p class="page-individual_copy">用途に合わせて利用できる2つのプランをご用意</p>
    <div class="page-individual_container">
        <div class="page-individual_card single">
            <p class="page-individual_card_plan">Plan01</p>
            <p class="page-individual_card_copy">個室利用ができる</p>
            <p class="page-individual_card_title">シングルオフィスプラン</p>
        </div>
        <div class="page-individual_card free">
            <p class="page-individual_card_plan">Plan02</p>
            <p class="page-individual_card_copy">共有スペースを自由に利用</p>
            <p class="page-individual_card_title">フリーオフィスプラン</p>
        </div>
    </div>
</div>

<!-- シングルオフィスプラン -->
<div class="page-plan single">
    <div class="page-plan_inner wrapper_sm">
        <p class="page-plan_copy">個室利用ができるシングルオフィスプラン</p>
        <div class="page-plan_text">
            <p class="page-plan_text_inner">
            シングルオフィスの説明をする。文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れています。この文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れていまこの文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れていま
            </p>
        </div>

        <!-- 料金 -->
        <div class="page-plan_price">
            <h2 class="page-section_main_title">Price</h2>
            <h3 class="page-section_sub_title">料金</h3>

            <table class="page-plan_table">
                <tr>
                    <td>月額</td>
                    <td> ¥50,000</td>
                </tr>
                <tr>
                    <td>初期費用</td>
                    <td>月額賃料2ヶ月分程度</td>
                </tr>
            </table>
        </div>
    </div>

    <?php include 'inc/facility-temp.php'; ?>

    <?php include 'inc/others-temp.php'; ?>
</div>

<!-- フリーオフィスプラン -->
<div class="page-plan free">
    <div class="page-plan_inner wrapper_sm">
        <p class="page-plan_copy">共有スペースを自由に利用できるフリーオフィスプラン</p>
        <div class="page-plan_text">
            <p class="page-plan_text_inner">
                フリーオフィスの説明をする。文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れています。この文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れていまこの文章はダミーです。文字の大きさ、量、字間、行間等を確認するために入れていま
            </p>
        </div>

        <!-- 料金 -->
        <div class="page-plan_price">
            <h2 class="page-section_main_title">Price</h2>
            <h3 class="page-section_sub_title">料金</h3>

            <table class="page-plan_table">
                <tr>
                    <td>月額</td>
                    <td>お問い合わせください</td>
                </tr>
                <tr>
                    <td>初期費用</td>
                    <td>月額賃料2ヶ月分程度</td>
                </tr>
            </table>
        </div>
    </div>
    
    <?php include 'inc/facility-temp.php'; ?>

</div>

<?php include 'inc/footer.php'; ?>